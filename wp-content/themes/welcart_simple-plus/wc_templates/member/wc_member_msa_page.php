			...
			<div id="memberpages">
				...
				<ul class="member_submenu">
					<li class="edit_member">
						<a href="<?php echo esc_url_raw( USCES_MEMBER_URL ); ?>">会員ページトップへ戻る</a>
					</li>
				...
				<h2><?php echo apply_filters( 'msa_filter_gift_label', 'ギフト用' ); ?>配送先を登録</h2>
				<div class="msa_area">
					<div class="msa_total">登録件数<span id="msa_num"></span>件<input id="new_destination" type="button" value="新しい配送先を追加する" /></div>
					<?php
					if ( isset( $_GET['msa_backurl'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended
						$welcart_simpleplus_cusurl = str_replace( 'customerinfo=1', 'customerinfo=msa', USCES_CUSTOMER_URL );
						?>
						<div class="return_navi"><a href="<?php echo esc_url_raw( $welcart_simpleplus_cusurl ); ?>">配送先設定へ戻る</a></div>
					...
					<?php do_action( 'msa_action_memberinfo_page_operation' ); //phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound ?>
					</div>
					<div class="msa_field_block">
						...
						<div class="msa_field">
							...
							<span class="name-ttl">姓</span><input id="msa_name" name="msa_name" type="text" />
							<span class="name-ttl">名</span><input id="msa_name2" name="msa_name2" type="text" />
							<span id="name_message" class="msa_message"></span>
						</div>
						<div class="msa_field">
							<label for="msa_furigana">フリガナ</label>
							<span class="name-ttl">セイ</span><input id="msa_furigana" name="msa_furigana" type="text" />
							<span class="name-ttl">メイ</span><input id="msa_furigana2" name="msa_furigana2" type="text" />
						</div>
						...
						<div class="msa_field">
							...
							<?php if ( isset( $usces->options['address_search'] ) && 'activate' === $usces->options['address_search'] ) : ?>
								<input type='button' id='search_zipcode' class='search-zipcode button' value='住所検索' onclick="AjaxZip3.zip2addr('msa_zip', '', 'msa_pref', 'msa_address1');">
							<?php endif; ?>
						...
						<input name="member_id" type="hidden" value="<?php echo esc_html( $usces_members['ID'] ); ?>"/>
						<div id="msa_button">
				...
				<div class="footer_explanation">
					<?php do_action( 'msa_action_memberinfo_page_footer' ); //phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound ?>
				</div>
