<?php
/**
 * Theme Name: Wc lostpassword Page
 *
 * @package Welcart
				...
				<p><?php esc_html_e( 'Change your password by following the instruction in this mail.', 'usces' ); ?></p>
					...
					<div class="loginbox">
						<form name="loginform" id="loginform" action="<?php usces_url( 'member' ); ?>" method="post" onKeyDown="if(event.keyCode == 13){return false;}">
							<p>
								<label><span><?php esc_html_e( 'e-mail adress', 'usces' ); ?></span><br />
								<input type="text" name="loginmail" id="loginmail" class="loginmail" value="" size="20" /></label>
							</p>
							<p class="submit">
								<input type="submit" name="lostpassword" id="member_login" value="<?php esc_html_e( 'Obtain new password', 'usces' ); ?>" />
							</p>
						...
						<p id="nav">
							<a href="<?php usces_url( 'login' ); ?>" title="<?php esc_html_e( 'Log-in', 'usces' ); ?>"><?php esc_html_e( 'Log-in', 'usces' ); ?></a>
						</p>
						<?php endif; ?>
					</div><!-- .loginbox -->
