			...
			if ( $i === 1 ) {
				...
				<div class="fep-per-message fep-per-message-top fep-per-message-<?php echo fep_get_the_id(); ?>">
					...
					<div style="text-align:center;">
					    ...
					    <!-- 改変 2019/10/7 -->
						<button class="fep-button fep-message-toggle-all" id="all_message" style=" background-color:skyblue; width:70%; max-width:300px; margin:20px 0 15px;"><?php esc_html_e( 'Toggle Messages', 'front-end-pm' ); ?></button>
					</div>
					<?php } ?>
				</div>
				<?php
				...
				<div class="fep-message-title fep-message-title-<?php echo fep_get_the_id(); ?>">
					...
					<div class="author">
					    <div style="display:flex; font-size:12px;">
					        <?PHP $p = fep_get_message_field( 'mgs_author' ); //アバター ?>
					        <!-- アバタークリックするとプロフィールページへ -->
					        <a href="https://<?PHP print $_SERVER["HTTP_HOST"].$_SERVER["SCRIPT_NAME"]; ?>/archives/author/<?PHP echo fep_user_name( fep_get_message_field( 'mgs_author' ) ); //ユーザー名 ?>">
					        <div class="message_avatar">
					            <?PHP echo get_avatar( $p, 60, '', fep_user_name( $p ), array( 'extra_attr'=> 'title="' . fep_user_name( $p ) . '"' ) ); ?>
					        </div>
					        </a>
					        <?PHP echo fep_user_name( fep_get_message_field( 'mgs_author' ) ); //ユーザー名 ?>
					        <br>
					        <?PHP echo fep_get_the_date(); //送信日時 ?>
					    </div><!-- Flexbox終わり -->
					</div>
					<!-- 改変終わり -->
...
window.onload = function(){
    document.getElementById("all_message").click();
}
</script>
<!-- 改変終わり -->