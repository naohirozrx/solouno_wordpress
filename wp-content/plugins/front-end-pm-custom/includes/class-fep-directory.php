	...
	function get_column_content( $column, $user ) {
		switch ( $column ) {
			...
			case 'fep-cb':
				...
				break;
			case 'avatar':
			    echo "<a href='https://".$_SERVER["HTTP_HOST"].$_SERVER["SCRIPT_NAME"]."/archives/author/".fep_user_name( $user->ID )."'>";
			...
			case 'block_unblock':
				...
				if ( get_current_user_id() != $user->ID ) {
					if ( fep_is_user_blocked_for_user( get_current_user_id(), $user->ID ) ) {
						echo '<a href="#" class="fep_block_unblock_user fep_user_blocked" data-user_id="' . $user->ID . '" data-user_name="' . esc_attr( fep_user_name( $user->ID ) ) . '">' . '<span style="margin:0 10px; color:blue;"><i class="fas fa-lock-open"></i></span>' . '</a>';
					} else {
						echo '<a href="#" class="fep_block_unblock_user" data-user_id="' . $user->ID . '" data-user_name="' . esc_attr( fep_user_name( $user->ID ) ) . '">' . '<span style="margin:0 10px; color:red;"><i class="fas fa-ban"></i></span>' . '</a>';
					}
				}
				break;
			case 'send_message' :
				if ( get_current_user_id() != $user->ID ) {
					?><a href="<?php echo fep_query_url( 'newmessage', array( 'fep_to' => $user->user_nicename ) ); ?>"><i class="fas fa-envelope fa-fw"></i></a><?php
				}
				break;
			default:
