...
class Fep_Admin_Pages {
	...
	function addAdminPage() {
		...
		add_menu_page( $label, 'DM機能設定', $admin_cap, 'fep-all-messages', array( $this, 'all_messages' ), 'dashicons-email', 30 );
		
		add_submenu_page( 'fep-all-messages', "$label - " . '全てのメッセージ', __( 'All Messages', 'front-end-pm' ), $admin_cap, 'fep-all-messages', array( $this, 'all_messages' ) );
		
		add_submenu_page( 'fep-all-messages', "$label - ". '全ての添付ファイル', __( 'All Attachments', 'front-end-pm' ), $admin_cap, 'fep-all-attachments', array( $this, 'all_attachments' ) );
