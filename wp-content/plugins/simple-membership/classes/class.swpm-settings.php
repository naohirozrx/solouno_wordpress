...
class SwpmSettings {
    ...
    private function tab_1() {
        ...
        add_settings_field('free-membership-id', SwpmUtils::_('Free Membership Level ID') . "「一般」", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-membership-id',
            'message' => SwpmUtils::_('Assign free membership level ID')));
        */
         
		// ================= kayama add 2019/12/24 Start ==============
		/*
		add_settings_field('free-youtuber-membership-id', "会員レベルの ID「Youtuber URL」", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-youtuber-membership-id',
            'message' => "会員のレベル ID を指定"));
		*/
		
		include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
        if(is_plugin_active( 'gianism/wp-gianism.php' ))  {
        
		add_settings_field('free-facebook-membership-id', "Facebookで登録した場合", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-facebook-membership-id',
            'message' => "Facebookで登録した場合の会員レベル"));
		
		add_settings_field('free-twitter-membership-id', "Twitterで登録した場合", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-twitter-membership-id',
            'message' => "Twitterで登録した場合の会員レベル"));
		
		add_settings_field('free-google-membership-id', "Googleで登録した場合", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-google-membership-id',
            'message' => "Googleで登録した場合の会員レベル"));
		
		add_settings_field('free-instagram-membership-id', "Instagramで登録した場合", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-instagram-membership-id',
            'message' => "Instagramで登録した場合の会員レベル"));
		
		add_settings_field('free-line-membership-id', "LINEで登録した場合", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-line-membership-id',
            'message' => "LINEで登録した場合の会員レベル"));
		
		add_settings_field('free-amazon-membership-id', "Amazonで登録した場合", array(&$this, 'textfield_small_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-amazon-membership-id',
            'message' => "Amazonで登録した場合の会員レベル"));
        }

		add_settings_field('free-membership-paycontent-error-id', "無料会員が有料コンテンツを閲覧した際の文言", array(&$this, 'textfield_long_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'free-membership-paycontent-error-id',
            'message' => "無料会員レベルが有料コンテンツページを見た場合"));

		add_settings_field('no-login-paycontent-error-id', "無料ユーザーが有料コンテンツを閲覧した際の文言", array(&$this, 'textfield_long_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'no-login-paycontent-error-id',
            'message' => "ログインしていなかった時に有料コンテンツページを見た場合"));
		// ========================== End ==================

        //改変 日本語化
        add_settings_field('enable-moretag', ('「続きを読む」以降を保護'), array(&$this, 'checkbox_callback'), 'simple_wp_membership_settings', 'general-settings', array('item' => 'enable-moretag',
            'message' => ('投稿やページの「続きを読む」による限定公開を有効にします。続きを読む以降のコンテンツが限定公開されます。')));
	...
	//改変
	/* 
	add_settings_field( 'advanced-user-login', 'ログインしていなかった時に有料コンテンツページを見た場合', array(
		&$this,
		'textarea_callback' ), 'simple_wp_membership_settings', 'general-settings', array(
		'item'		 => 'advanced-user-login',
		'message'	 => '' ) );
	add_settings_field( 'advanced-user-level', 'ログインしているが会員レベルがそのページを閲覧できる権限でない場合', array(
		&$this,
		'textarea_callback' ), 'simple_wp_membership_settings', 'general-settings', array(
		'item'		 => 'advanced-user-level',
		'message'	 => '' ) );
	*/
    ...
    private function tab_3() {
        ...
        add_settings_field('disable-email-after-upgrade', SwpmUtils::_('アップグレード後の電子メール通知を無効にする'), array(&$this, 'checkbox_callback'), 'simple_wp_membership_settings', 'upgrade-email-settings', array('item' => 'disable-email-after-upgrade',
            'message' => SwpmUtils::_('メンバーがアップグレードまたは更新のために支払いを行ったときにメンバーに送信される電子メール通知を無効にする。')));
    ...
    public function swpm_documentation_callback() {
        ...
        <div class="swpm-orange-box">
            <?php printf(SwpmUtils::_('設定方法については<a href="https://www.evernote.com/l/AVrcHkyLK3xHRoa5up33iEA47xsBgBYUdaU">こちらのノート</a>をご覧ください。<br>リンクが無効の場合は、お手数ですが別途お送りする有料会員設定についてのマニュアルをご覧頂くか、お問い合わせください。')); ?>
        </div>
        <?php
    }
    ...
    public function email_settings_overview_callback() {
        ...
        echo('この欄の設定は基本的に不要です。アップグレード時に配信されるメールのみを設定してください。');
        echo '</div>';
    }
    ...
    public function sanitize_tab_1($input) {
        ...
        $output['free-membership-id'] = ($input['free-membership-id'] != 1) ? absint($input['free-membership-id']) : '2';

		// ==================== kayama Add 2019/12/24 ================
		// $output['free-youtuber-membership-id'] = ($input['free-youtuber-membership-id'] > 1) ? absint($input['free-youtuber-membership-id']) : '2';
		$output['free-facebook-membership-id'] = ($input['free-facebook-membership-id'] > 1) ? absint($input['free-facebook-membership-id']) : '2';
		$output['free-twitter-membership-id'] = ($input['free-twitter-membership-id'] > 1) ? absint($input['free-twitter-membership-id']) : '2';
		$output['free-google-membership-id'] = ($input['free-google-membership-id'] > 1) ? absint($input['free-google-membership-id']) : '2';
		$output['free-instagram-membership-id'] = ($input['free-instagram-membership-id'] > 1) ? absint($input['free-instagram-membership-id']) : '2';
		$output['free-line-membership-id'] = ($input['free-line-membership-id'] > 1) ? absint($input['free-line-membership-id']) : '2';
		$output['free-amazon-membership-id'] = ($input['free-amazon-membership-id'] > 1) ? absint($input['free-amazon-membership-id']) : '2';

		$output['free-membership-paycontent-error-id'] = !empty($input['free-membership-paycontent-error-id']) ? ($input['free-membership-paycontent-error-id']) : 'このコンテンツは有料会員様限定です。';
		$output['no-login-paycontent-error-id'] = !empty($input['no-login-paycontent-error-id']) ? ($input['no-login-paycontent-error-id']) : 'このコンテンツは有料会員様限定です。';
		// ========================= End ===========================

	/* 改変 */
	/*
	$output[ 'advanced-user-level' ] = isset( $input[ 'advanced-user-level' ] ) ? $input[ 'advanced-user-level' ] : "";
	$output[ 'advanced-user-login' ] = isset( $input[ 'advanced-user-login' ] ) ? $input[ 'advanced-user-login' ] : "";
	*/
