...
class SwpmPostList extends WP_List_Table {
    ...
    function column_taxonomy($item) {
        ...
        if ($taxonomy == 'category'){
            $taxonomy = SwpmUtils::_('Post Category'); //kayama change 2019/12/24
        } else {
            $taxonomy = SwpmUtils::_('Custom Post Type').' ('.SwpmUtils::_($taxonomy).')'; //kayama change 2019/12/24
        }
        ...
        switch ($this->type) {
            case 'page':
                ...
                foreach ($all_pages as $page) {
                    ...
                    $page_summary['status'] = __(ucfirst($page->post_status));
            ...
            case 'post':
                ...
                foreach ($all_posts as $post) {
                    ...
                    $post_summary['status'] = __(ucfirst($post->post_status));
            ...
            case 'custom_post':
                ...
                if (!empty($arg)) {
                    ...
                    foreach ($all_posts as $post) {
                        ...
                        $post_summary['title'] = '<a href="' . get_permalink($post->ID) . '" target="_blank">' . $post->post_title . '</a>';
                        $post_summary['author'] = $user_info->user_login;
                        $post_summary['type'] = __(ucfirst($post->post_type));
                        $post_summary['status'] = __(ucfirst($post->post_status));
                        $filtered_items[] = $post_summary;
                    }
                }
