...
class SimpleWpMembership {
    ...
    public function sync_with_wp_profile($wp_user_id) {
        ...
        $profile['user_name'] = $wp_user_data->user_email; // $profile['user_name'] = $wp_user_data->user_login; // kayama change 2019/12/27 [because change user_login to user_email in user.php]
    ...
    function swpm_handle_wp_user_registration($user_id) {
		...
		// get giansim login type
		$login_type = isset($giansim_options["login_type"])?strtolower($giansim_options["login_type"]):"";// $login_type = strtolower($swpm_settings_obj->get_value("login_type"));
		if(!empty($login_type)){
			//Facebook, Google, Instagram, LINE, Amazon, ?
			if($login_type == "facebook"){
				$default_level = $swpm_settings_obj->get_value("free-facebook-membership-id", 2);
			}else if($login_type == "google"){
				$default_level = $swpm_settings_obj->get_value("free-google-membership-id", 2);
			}else if($login_type == "instagram"){
				$default_level = $swpm_settings_obj->get_value("free-instagram-membership-id", 2);
			}else if($login_type == "line"){
				$default_level = $swpm_settings_obj->get_value("free-line-membership-id", 2);
			}else if($login_type == "amazon"){
				$default_level = $swpm_settings_obj->get_value("free-amazon-membership-id", 2);
			}else if($login_type == "twitter"){
				$default_level = $swpm_settings_obj->get_value("free-twitter-membership-id", 2);
			}else if($login_type == "youtube"){
				$default_level = $swpm_settings_obj->get_value("free-youtuber-membership-id", 3);
			}else{
				$default_level = $swpm_settings_obj->get_value("free-membership-id", 2);
			}
		}
		// ================= End ===================
