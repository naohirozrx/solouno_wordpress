...
class SwpmAccessControl {
    ...
    public function can_i_read_post($post){
        ...
        //改変
        $domain = $_SERVER['SERVER_NAME'].$_SERVER["SCRIPT_NAME"];

		// ================== kayama add & change 2019/12/24 ==================
		$swpm_settings_obj = SwpmSettings::get_instance();

        if(!$auth->is_logged_in()){
			if (is_user_logged_in()) {
				$message = $swpm_settings_obj->get_value("free-membership-paycontent-error-id");
				$text = "<br>".$message."<div id='sm_register_button'><a href='https://$domain/register'>有料会員登録</a></div>"; // $text = "<br>このコンテンツは有料会員様限定です。<div id='sm_register_button'><a href='https://$domain/register'>有料会員登録</a></div>";
				// $text = SwpmSettings::get_instance()->get_value( 'advanced-user-level', "<br>このコンテンツは有料会員様限定です。<div id='sm_register_button'><a href='https://$domain/register'>有料会員登録</a></div>" );
			} else {
				$message = $swpm_settings_obj->get_value("no-login-paycontent-error-id");
				$text = "<br>".$message."<div id='sm_login_button'><a href='https://$domain/?memberpage=login'>ログイン</a></div>"; // $text = "<br>このコンテンツは有料会員様限定です。<div id='sm_login_button'><a href='https://$domain/?memberpage=login'>ログイン</a></div>";
				// $text = SwpmSettings::get_instance()->get_value( 'advanced-user-login', "<br>.このコンテンツは有料会員様限定です。<div id='sm_login_button'><a href='https://$domain/?memberpage=login'>ログイン</a></div>" );
			}
			$error_msg = '<div style="text-align:center; margin-bottom:10px;" class="swpm-post-not-logged-in-msg">'.$text.'</div>';
        ...
        if($permission->is_permitted($id)) {
			...
			$message = $swpm_settings_obj->get_value("free-membership-paycontent-error-id"); // kayama add 2019.12.24
            $text = SwpmUtils::_("<div style='text-align:center;'>".$message."<br><a href='https://$domain/register'>有料会員登録</a></div>"); // $text = SwpmUtils::_("<div style='text-align:center;'>このコンテンツは有料会員様限定です。<br><a href='https://$domain/register'>有料会員登録</a></div>");
	    // $text			 = SwpmSettings::get_instance()->get_value( 'advanced-user-level', "<br>このコンテンツは有料会員様限定です。<div id='sm_register_button'><a href='https://$domain/register'>有料会員登録</a></div>" );
