...
class SwpmInstallation {
    ...
    public static function installer() {
		...
		// ============= kayama p-1 add 2020/1/14 start ===============
		$sql = "SELECT * FROM " . $wpdb->prefix . "swpm_membership_tbl WHERE id = 2";
        $results = $wpdb->get_row($sql);
		if (is_null($results)) {
			$sql = "INSERT INTO  " . $wpdb->prefix . "swpm_membership_tbl  (
			id ,
			...
			options,
			campaign_name
			)VALUES (2 , '無料会員', 'contributor', '0', '0', NULL, NULL, NULL , NULL , NULL , NULL,NULL,NULL,'');";
            $wpdb->query($sql);
		}
		// =================== End =================
    ...
    public static function initdb() {
        ...
        $reg_prompt_email_subject = "新規登録を完了してください";
        $reg_prompt_email_body = "{first_name} {last_name}様" .
                "\n\nご登録いただき、有難うございます。" .
                "\n\n次のリンクにアクセスして、登録を完了してください。(既に登録が完了している場合は、本メールを無視してください)" .
                "\n\n{reg_link}";
        $reg_email_subject = "会員登録が完了しました";
        $reg_email_body = "{first_name} {last_name}様\n\n" .
                "新規会員登録が完了致しました。";
        $reg_email_subject_admin = "新規会員登録者がありました。";
        $reg_email_body_admin = "新規会員登録がありました。\n\n" .
                "ユーザーネーム: {user_name}\n";
        $upgrade_email_subject = "アカウントがアップグレードされました！";
        $upgrade_email_body = "{first_name} {last_name}様" .
                "\n\n決済が完了し、アカウントがアップグレードされました。決済の詳細はご利用したPayPalアカウントをご確認ください。";
        $reset_email_subject = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";
        $reset_email_body = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";

        $status_change_email_subject = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";
        $status_change_email_body = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";

        $bulk_activate_email_subject = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";
        $bulk_activate_email_body = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";

        $email_activation_mail_subject = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";
        $email_activation_mail_body = "この欄の記入は不要です。Simple Membershipを用いて登録させる場合は記入してください。";
		...
		// ============= kayama p-1 add 2020/1/15 start ===============
		SwpmMiscUtils::delete_mandatory_wp_pages();

		$settings->set_value('auto-create-default-membership-level', "2");
		$settings->set_value('force-wp-user-sync', stripslashes("checked=\"checked\""));
		$settings->set_value('enable-auto-create-swpm-members', stripslashes("checked=\"checked\""));	
		// =================== End ==============
