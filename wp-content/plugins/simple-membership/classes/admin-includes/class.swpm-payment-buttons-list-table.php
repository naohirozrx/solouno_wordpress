...
class SwpmPaymentButtonsListTable extends WP_List_Table {
    ...
    function column_ID($item) {
        ...
        //Build row actions
        $actions = array(
            'edit' => sprintf('<a href="admin.php?page=simple_wp_membership_payments&tab=edit_button&button_id=%s&button_type=%s">'.__("Edit").'</a>', $item['ID'], $button_type),
            'delete' => sprintf('<a href="admin.php?page=simple_wp_membership_payments&tab=payment_buttons&action=delete_payment_btn&button_id=%s" onclick="return confirm(\''.SwpmUtils::_('Are you sure you want to delete this record?').'\')">'.__("Delete").'</a>', $item['ID']), // kayama change
        );
    ...
    function get_columns() {
        $columns = array(
            ...
            'button_shortcode' => SwpmUtils::_('ショートコード<br>(このコードをお好きな場所に貼り付けてください)'),
        );
        return $columns;
    }
