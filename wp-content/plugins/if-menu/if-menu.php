<?php
/*
Plugin Name: If Menu(Ver.BuildSalon)
Description: メニューに表示条件を追加します
*/
...
class If_Menu {
	...
	public static function menu_item_fields($item_id) {
		...
		<p class="if-menu-enable description description-wide">
			<a href="<?php echo admin_url('themes.php?page=if-menu') ?>" class="if-menu-help" data-tooltip="<?php esc_attr_e('表示条件のサンプルを見る', 'if-menu') ?>" title="<?php esc_attr_e('表示条件のサンプルを見る', 'if-menu') ?>" target="_blank"><span class="dashicons dashicons-editor-help"></span></a>
			<label>
				...
				<?php esc_html_e('表示条件を有効にする', 'if-menu') ?>
			</label>
		</p>
				...
				<p class="if-menu-condition description description-wide" data-menu-item-id="<?php echo $item_id ?>">
					...
					<span class="if-menu-condition-rule">
						<select class="menu-item-if-menu-condition-type" id="edit-menu-item-if-menu-condition-type-<?php echo esc_attr( $item_id ); ?>" name="menu-item-if-menu-condition-type[<?php echo esc_html( $item_id ); ?>][]" data-val="<?php echo esc_html($if_menu_condition_type[$index]) ?>">
							<option <?php selected( 'show', $if_menu_condition_type[$index] ) ?> value="show"><?php esc_html_e( '表示する', 'if-menu' ) ?></option>
							<option <?php selected( 'hide', $if_menu_condition_type[$index] ) ?> value="hide"><?php esc_html_e( '非表示にする', 'if-menu' ) ?></option>
						</select>
					...
					<select class="menu-item-if-menu-enable-next" name="menu-item-if-menu-enable[<?php echo esc_attr( $item_id ); ?>][]">
						...
						<option value="and" <?php if (isset($if_menu_enable[$index + 1])) selected( 'and', $if_menu_enable[$index + 1] ) ?>><?php esc_html_e( 'および', 'if-menu' ) ?></option>
						<option value="or" <?php if (isset($if_menu_enable[$index + 1])) selected( 'or', $if_menu_enable[$index + 1] ) ?>><?php esc_html_e( 'または', 'if-menu' ) ?></option>-->
					</select>
  ...
  public static function menu_item_title( $item_id ) {
    ...
    if ( count( $if_menu_enabled ) && $if_menu_enabled[0] !== '0' ) {
      ...
      echo '<span class="is-submenu">表示条件があります</span>';
    }
  }
