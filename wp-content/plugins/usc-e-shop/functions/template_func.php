	...
	foreach ( (array) $payments as $id => $payment ) {
		...
		if ( '' !== $payment['name'] && 'deactivate' !== $payment['use'] ) {
			...
			if ( ! empty( $explanation ) ) {
				$explanation = usces_remove_script( $explanation );
...
function usces_load_filter_purchase_date() {
	...
	$content      .= '<input type="checkbox" id="ord_exclude_cancel" name="ord_exclude_cancel" onchange="this.disabled=true;location.href=this.value;" value="' . esc_url( $url_ex_filter ) . '" ' . checked( $exclude_cancel, 'on', false ) . ' />';
	$content      .= __( 'Exclude cancellations', 'usces' ) . '</label>';
	$content      .= '</div>';
	$content      .= '<div class="usce_period">';
	$content      .= '<span>' . __( 'Period', 'usces' ) . '</span>';
	$content      .= '<select name="usces_purdate" id="usces_purdate" onchange="location.href=this.options[this.selectedIndex].value;">';
	foreach ( $arr_filter as $key => $name ) {
		$selected = '';
		if ( empty( $key ) ) {
			$url_filter = USCES_MEMBER_URL . '#usces_history';
		} else {
			$add_args = array(
				'pur-date' => $key,
			);
		...
		$content .= '<option value="' . esc_url( $url_filter ) . '" ' . esc_attr( $selected ) . '>' . esc_attr( $name ) . '</option>';
	}
	$content .= '</select>';
	$content .= '</div>';
...
function usces_get_cart_rows( $out = '' ) {
	...
	for ( $i = 0; $i < $cart_count; $i++ ) {
		...
		$stock_column = '<td ' . $red . '>' . esc_html( $stock ) . '</td>';
