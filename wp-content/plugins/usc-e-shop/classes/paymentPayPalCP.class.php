...
class PAYPAL_CP_SETTLEMENT {
	...
	public function admin_scripts() {
		...
		$admin_page = ( isset( $_GET['page'] ) ) ? wp_unslash( $_GET['page'] ) : ''; // phpcs:ignore
		switch ( $admin_page ) :
			/* クレジット決済設定画面 */
	...
	$( document ).on( "click", "#upfront_onboarding_paypal_cp", function() {
		...
		return false;
			...
			case 'usces_continue':
				$order_id        = '';
				$acting_flg      = '';
				$order_action    = ( isset( $_GET['order_action'] ) ) ? wp_unslash( $_GET['order_action'] ) : ''; // phpcs:ignore
				$continue_action = ( isset( $_GET['continue_action'] ) ) ? wp_unslash( $_GET['continue_action'] ) : ''; // phpcs:ignore
				if ( ( 'usces_orderlist' == $admin_page && ( 'edit' == $order_action || 'editpost' == $order_action || 'newpost' == $order_action ) ) ||
					...
					$order_id = ( isset( $_REQUEST['order_id'] ) ) ? wp_unslash( $_REQUEST['order_id'] ) : ''; // phpcs:ignore
					if ( ! empty( $order_id ) ) {
				...
				if ( in_array( $acting_flg, $this->pay_method ) ) :
					...
					if ( isset( $usces_settings['currency'][ $cr ] ) ) {
						...
						if ( 'JPY' === $code ) {
							$currency = __( $code, 'usces' ); // phpcs:ignore
						} else {
...
jQuery( document ).ready( function( $ ) {
	...
	$( document ).on( "click", "#capture-settlement", function() {
		...
		if ( amount_capture == amount_original ) {
			...
			if ( ! confirm( <?php printf( __( "'Execute the refund payment process for %s' + amount_refund + '. Are you sure?'", 'usces' ), $currency ); // phpcs:ignore ?> ) ) {
				return;
			}
		}
	...
	$( document ).on( "click", "#re-authorize-settlement", function() {
		...
		if ( ! confirm( <?php printf( __( "'Execute the void payment process for %s' + amount_authorize + '. Are you sure?'", 'usces' ), $currency ); // phpcs:ignore ?> ) ) {
			return;
		}
	...
	$( document ).on( "click", "#re-capture-settlement", function() {
		...
		if ( ! confirm( <?php printf( __( "'Execute the capture payment process for %s' + amount_capture + '. Are you sure?'", 'usces' ), $currency ); // phpcs:ignore ?> ) ) {
			return;
		}
	...
	public function settlement_update() {
		...
		if ( $this->paymod_id != wp_unslash( $_POST['acting'] ) ) { // phpcs:ignore
			return;
		}
		...
		if ( '' === $this->error_mes ) {
			...
			if ( 'on' === $options['acting_settings']['paypal_cp']['cp_activate'] || 'on' === $options['acting_settings']['paypal_cp']['card_activate'] ) {
				...
				if ( 'on' === $options['acting_settings']['paypal_cp']['card_activate'] ) {
					...
					foreach ( $payment_method as $settlement => $payment ) {
						if ( 'acting_paypal_card' == $settlement && 'deactivate' === $payment['use'] ) {
							$toactive[] = $payment['name'];
						}
					}
				}
				...
				if ( 0 < count( $deactivate ) ) {
					$deactivate_message     = sprintf( __( '"Deactivate" %s of payment method.', 'usces' ), implode( ',', $deactivate ) ); // phpcs:ignore
	...
	public function settlement_tab_body() {
		...
		if ( in_array( $this->paymod_id, (array) $settlement_selected ) ) :
			...
			if ( isset( $acting_opts['cp_activate'] ) && 'on' === $acting_opts['cp_activate'] ) {
				$cp_activate_on = ' checked';
			} else {
				$cp_activate_off = ' checked';
			}
			$card_activate_off = '';
			$card_activate_on  = '';
			if ( isset( $acting_opts['card_activate'] ) && 'on' === $acting_opts['card_activate'] ) {
				$card_activate_on = ' checked';
			} else {
				$card_activate_off = ' checked';
			}
			$card_vault_off = '';
			$card_vault_on  = '';
			if ( isset( $acting_opts['card_vault'] ) && 'on' === $acting_opts['card_vault'] ) {
				$card_vault_on = ' checked';
			} else {
				$card_vault_off = ' checked';
			}
			$card_3ds_off = '';
			$card_3ds_on  = '';
			...
			if ( isset( $acting_opts['button_layout'] ) && 'horizontal' === $acting_opts['button_layout'] ) {
				$cp_button_layout_horizontal = ' checked';
			} else {
				$cp_button_layout_vertical = ' checked'; /* default */
			}
			$class_button_color     = '';
			...
			if ( isset( $acting_opts['button_shape'] ) && 'pill' === $acting_opts['button_shape'] ) {
				$cp_button_shape_pill = ' checked';
				$class_button_shape   = ' shape-pill';
			} else {
				$cp_button_shape_rect = ' checked'; /* default */
	...
	<div class="settlement_service"><span class="service_title"><?php esc_html_e( 'PayPal Commerce Platform', 'usces' ); ?></span></div>
			<?php
			if ( isset( $_POST['acting'] ) && $this->paymod_id == $_POST['acting'] ) : // phpcs:ignore
				if ( '' != $this->error_mes ) :
					?>
		<div class="error_message"><?php echo $this->error_mes; // phpcs:ignore ?></div>
					<?php
	...
	<form action="" method="post" name="paypal_cp_form" id="paypal_cp_form">
		<table class="settle_table">
			...
			<tr>
				...
				<td><label><input type="radio" class="activate_paypal_card" name="card_activate" value="on"<?php echo esc_attr( $card_activate_on ); ?>/><span><?php esc_html_e( 'Use', 'usces' ); ?></span></label><br />
					<label><input type="radio" class="activate_paypal_card" name="card_activate" value="off"<?php echo esc_attr( $card_activate_off ); ?>/><span><?php esc_html_e( 'Do not Use', 'usces' ); ?></span></label>
				</td>
			</tr>
			...
			<tr class="paypal_card_form">
				...
				<td><label><input type="radio" name="card_vault" value="on"<?php echo esc_attr( $card_vault_on ); ?>/><span><?php esc_html_e( 'Save', 'usces' ); ?></span></label><br />
					<label><input type="radio" name="card_vault" value="off"<?php echo esc_attr( $card_vault_off ); ?>/><span><?php esc_html_e( 'Not save', 'usces' ); ?></span></label>
				</td>
			</tr>
			...
			<!--<tr class="paypal_card_form">
				<th><a class="explanation-label" id="label_ex_card_3ds_paypal_cp"><?php esc_html_e( '3D Secure', 'usces' ); ?></a></th>
				<td><label><input type="radio" name="card_3ds" value="on"<?php echo esc_attr( $card_3ds_on ); ?>/><span><?php esc_html_e( 'Use', 'usces' ); ?></span></label><br />
					<label><input type="radio" name="card_3ds" value="off"<?php echo esc_attr( $card_3ds_off ); ?>/><span><?php esc_html_e( 'Do not Use', 'usces' ); ?></span></label>
				</td>
			</tr>
			<tr id="ex_card_3ds_paypal_cp" class="explanation paypal_card_form"><td colspan="2"></td></tr>-->
...
クレジットカード決済の申込みは<a href="https://www.paypal.com/jp/webapps/mpp/contact-sales">こちら</a>から行えます。</p>
	...
	public function delivery_check( $mes ) {
		if ( ! isset( $_POST['offer']['payment_name'] ) ) { // phpcs:ignore
			return $mes;
		}

		$payment = usces_get_payments_by_name( $_POST['offer']['payment_name'] ); // phpcs:ignore
		if ( isset( $payment['settlement'] ) && 'acting_paypal_card' === $payment['settlement'] ) {
			if ( isset( $_POST['paypal_payment_card'] ) && 'vault' == wp_unslash( $_POST['paypal_payment_card'] ) ) { // phpcs:ignore
			} else {
				if ( isset( $_POST['paypal_resource_id'] ) && empty( $_POST['paypal_resource_id'] ) ) { // phpcs:ignore
	...
	public function point_inform( $html ) {
		...
		if ( isset( $payment['settlement'] ) && 'acting_paypal_card' === $payment['settlement'] ) {
			if ( isset( $_POST['paypal_tracking_id'] ) ) { // phpcs:ignore
				$html .= '<input type="hidden" name="paypal_tracking_id" value="' . $_POST['paypal_tracking_id'] . '">'; // phpcs:ignore
			}
			if ( isset( $_POST['paypal_resource_id'] ) ) { // phpcs:ignore
				$html .= '<input type="hidden" name="paypal_resource_id" value="' . $_POST['paypal_resource_id'] . '">'; // phpcs:ignore
			}
			if ( isset( $_POST['paypal_payment_card'] ) ) { // phpcs:ignore
				$html .= '<input type="hidden" name="paypal_payment_card" value="' . $_POST['paypal_payment_card'] . '">'; // phpcs:ignore
			}
			if ( isset( $_POST['paypal_payment_token_id'] ) ) { // phpcs:ignore
				$html .= '<input type="hidden" name="paypal_payment_token_id" value="' . $_POST['paypal_payment_token_id'] . '">'; // phpcs:ignore
			}
			if ( isset( $_POST['paypal_vault'] ) ) { // phpcs:ignore
				$html .= '<input type="hidden" name="paypal_vault" value="' . $_POST['paypal_vault'] . '">'; // phpcs:ignore
			}
			if ( isset( $_POST['paypal_f'] ) ) { // phpcs:ignore
				$html .= '<input type="hidden" name="paypal_f" value="' . $_POST['paypal_f'] . '">'; // phpcs:ignore
			}
		}
		return $html;
	}
	...
	public function confirm_inform( $form, $payment, $acting_flg, $rand, $purchase_disabled ) {
		...
		if ( 'acting_paypal_cp' === $acting_flg ) {
			...
			$tracking_id      = ( isset( $_POST['paypal_tracking_id'] ) ) ? $_POST['paypal_tracking_id'] : ''; // phpcs:ignore
			$resource_id      = ( isset( $_POST['paypal_resource_id'] ) ) ? $_POST['paypal_resource_id'] : ''; // phpcs:ignore
			$payment_card     = ( isset( $_POST['paypal_payment_card'] ) ) ? $_POST['paypal_payment_card'] : ''; // phpcs:ignore
			$payment_token_id = ( isset( $_POST['paypal_payment_token_id'] ) ) ? $_POST['paypal_payment_token_id'] : ''; // phpcs:ignore
			$vault            = ( isset( $_POST['paypal_vault'] ) ) ? $_POST['paypal_vault'] : ''; // phpcs:ignore
			$correlation_id   = ( isset( $_POST['paypal_f'] ) ) ? $_POST['paypal_f'] : ''; // phpcs:ignore
	...
	public function enqueue_scripts() {
		...
		if ( ! is_admin() && $this->is_validity_acting( 'card' ) ) {
			if ( ( $usces->is_cart_page( $_SERVER['REQUEST_URI'] ) && 'delivery' === $usces->page ) ||
				...
				wp_enqueue_style( 'usces-paypal-style', $paypal_style_url, array() ); // phpcs:ignore
				wp_enqueue_style( 'usces-loading', USCES_FRONT_PLUGIN_URL . '/css/loading.css', array() ); // phpcs:ignore
			}
		}
	}
...
<script type="application/json" fncls="fnparams-dede7cc5-15fd-4c75-a9f4-36c430ee3a99">
{
	...
	"s":"<?php echo self::API_FRAUDNET_S; ?>"<?php if ( 'live' != $cp_environment ) : // phpcs:ignore ?>,
	"sandbox":true
<?php endif; ?>}
</script>
...
<script src="https://www.paypal.com/sdk/js?<?php echo $sdk_query; ?>" data-client-token="<?php echo $client_token; // phpcs:ignore ?>"></script>
<script>
jQuery( document ).ready( function($) {
...
<script>
	...
	let paypal_card_name = "<?php echo $paypal_card_name; // phpcs:ignore ?>";
	let orderId;
	if ( paypal.HostedFields.isEligible() ) {
		paypal.HostedFields.render({
			...
			document.querySelector( '#delivery-form' ).addEventListener( 'submit', event => {
				...
				var select_vault = "";<?php endif; ?>
				if ( 'to_confirm_button' == event.target.ownerDocument.activeElement.className && paypal_card_name == select_payment && "vault" != select_vault ) {
					document.getElementById( "welcart-overlay" ).style.display = 'block';
					event.preventDefault();
					hostedFieldsInstance.submit(<?php if ( 'on' === $card_vault ) : // phpcs:ignore ?>{
		...
		elseif ( 'confirm' == $usces->page ) :
			...
			if ( 'acting_paypal_cp' == $payment['settlement'] && 'activate' == $payment['use'] ) :
				...
				$sdk_query = http_build_query( $query );
				?>
<script src="https://www.paypal.com/sdk/js?<?php echo $sdk_query; // phpcs:ignore ?>"></script>
<script>
	paypal.Buttons({
		style: {
		...
		elseif ( $usces->is_member_page( $_SERVER['REQUEST_URI'] ) ) : // phpcs:ignore
			$member = $usces->get_member();
...
jQuery( document ).ready( function( $ ) {
	$( "input[name='deletemember']" ).css( "display", "none" );
			...
			if ( 'member_update_settlement' === filter_input( INPUT_GET, 'usces_page', FILTER_SANITIZE_STRING ) || 'member_register_settlement' === filter_input( INPUT_GET, 'usces_page', FILTER_SANITIZE_STRING ) ) :
				$acting_opts  = $this->get_acting_settings();
				$cp_client_id = ( isset( $acting_opts['client_id'] ) ) ? $acting_opts['client_id'] : '';
				$card_vault   = ( isset( $acting_opts['card_vault'] ) ) ? $acting_opts['card_vault'] : 'off';
				if ( 'on' !== $card_vault ) {
...
<script>
	...
	if ( paypal.HostedFields.isEligible() ) {
		paypal.HostedFields.render({
			...
			document.querySelector( '#member-card-update' ).addEventListener( 'submit', event => {
				document.getElementById( "update_mode" ).value = event.target.ownerDocument.activeElement.id;
				if ( 'card-register' == event.target.ownerDocument.activeElement.id || 'card-update' == event.target.ownerDocument.activeElement.id ) {
					document.getElementById( "welcart-overlay" ).style.display = 'block';
					event.preventDefault();
	...
	public function onboarded() {
		...
		$auth_code    = $_POST['authCode']; // phpcs:ignore
		$shared_id    = $_POST['sharedId']; // phpcs:ignore
		$seller_nonce = $_POST['seller_nonce']; // phpcs:ignore
	...
	public function create_billing_agreement() {
		...
		$tracking_id = wp_unslash( $_POST['tracking_id'] ); // phpcs:ignore
		$billing     = wp_unslash( $_POST['billing'] ); /* continue|regular */ // phpcs:ignore
	...
	public function acting_processing( $acting_flg, $post_query ) {
		...
		if ( 'acting_paypal_cp' === $acting_flg ) {
			...
			if ( $billing_token ) {
				...
				if ( isset( $response_billing_agreements['response']['code'] ) && '201' == $response_billing_agreements['response']['code'] && isset( $response_billing_agreements_data['id'] ) ) {
					...
					if ( isset( $response_order_data['id'] ) && isset( $response_order_data['status'] ) && 'CREATED' == $response_order_data['status'] ) {
						...
						if ( isset( $response_cancel_agreements['response']['code'] ) && '200' == $response_cancel_agreements['response']['code'] ) {
				...
				if ( isset( $response_order_data['intent'] ) && $cp_intent == $response_order_data['intent'] && isset( $response_order_data['status'] ) && 'APPROVED' == $response_order_data['status'] ) { // phpcs:ignore
				} else {
			...
			if ( 'vault' == $payment_card && ! empty( $payment_token_id ) ) {
				...
				if ( isset( $response['response']['code'] ) && '204' == $response['response']['code'] ) { // phpcs:ignore
	...
	public function api_authorize_void( $access_token, $bn_code, $tracking_id, $id ) {
		...
		} elseif ( isset( $response['response']['code'] ) && '204' == $response['response']['code'] ) {
	...
	public function member_update_settlement() {
		...
		if ( $usces->is_member_page( $_SERVER['REQUEST_URI'] ) ) { // phpcs:ignore
	...
	public function member_update_settlement_form() {
		...
		$update_mode = ( isset( $_POST['update_mode'] ) ) ? wp_unslash( $_POST['update_mode'] ) : ''; // phpcs:ignore
		if ( 'card-register' == $update_mode ) {
			check_admin_referer( 'member_update_settlement', 'wc_nonce' );
			$check_verify_recaptcha = $usces->verifyGoogleRecapcha();
			if ( ! $check_verify_recaptcha ) {
				$usces->error_message .= '<p>' . __( 'Could not send successfully. Please try again.', 'usces' ) . '</p>';
			} else {
				$api_request_url = ( isset( $acting_opts['api_request_url'] ) ) ? $acting_opts['api_request_url'] : '';
				$tracking_id     = ( isset( $_POST['paypal_tracking_id'] ) ) ? $_POST['paypal_tracking_id'] : ''; // phpcs:ignore
				$resource_id     = ( isset( $_POST['paypal_resource_id'] ) ) ? $_POST['paypal_resource_id'] : ''; // phpcs:ignore
			...
			if ( ! $check_verify_recaptcha ) {
				...
				$tracking_id      = ( isset( $_POST['paypal_tracking_id'] ) ) ? $_POST['paypal_tracking_id'] : ''; // phpcs:ignore
				$resource_id      = ( isset( $_POST['paypal_resource_id'] ) ) ? $_POST['paypal_resource_id'] : ''; // phpcs:ignore
				$payment_token_id = ( isset( $_POST['paypal_payment_token_id'] ) ) ? wp_unslash( $_POST['paypal_payment_token_id'] ) : ''; // phpcs:ignore
				if ( ! empty( $payment_token_id ) ) {
...
<div id="content" class="two-column">
				...
				<div id="memberpages">
					<div class="whitebox">
						<div id="memberinfo">
							...
							<div class="error_message"><?php echo $error_message; // phpcs:ignore ?></div>
							<form id="member-card-update" name="member_card_update" action="<?php echo esc_url( $update_settlement_url ); ?>" method="post" onKeyDown="if(event.keyCode == 13) {return false;}">
								...
								<input type="hidden" id="paypal_resource_id" name="paypal_resource_id" value="" />
								<input type="hidden" id="paypal_payment_token_id " name="paypal_payment_token_id" value="<?php echo esc_attr( $payment_token_id ); ?>" />
								<input type="hidden" id="update_mode" name="update_mode" value="" />
								<div class="send">
									<?php if ( $register ) : ?>
									<input type="submit" id="card-register" class="card-update" value="<?php esc_attr_e( 'Register', 'usces' ); ?>" />
									<?php else : ?>
	...
	public function admin_ajax() {
		...
		$mode = sanitize_title( $_POST['mode'] ); // phpcs:ignore
		$data = array();
		...
		switch ( $mode ) {
			...
			case 'get_paypal_cp':
				...
				if ( $response_data ) {
					if ( isset( $response_data['name'] ) && isset( $response_data['message'] ) && isset( $response_data['debug_id'] ) ) {
						...
						$result       .= '<div class="paypal-settlement-admin' . $class . '">' . __( $acting_status, 'usces' ) . '</div>'; // phpcs:ignore
						...
						$result        .= '<div class="paypal-settlement-admin' . $class . '">' . __( $acting_status, 'usces' ) . $acting_pending . '</div>'; // phpcs:ignore
						if ( 0 < $amount ) {
			...
			case 'capture_paypal_cp':
				...
				$result               .= $this->settlement_history( $order_id, $tracking_id );
				$data['result']        = $result;
				$data['acting_status'] = '<span class="acting-status' . $class . '">' . __( $acting_status, 'usces' ) . '</span>'; // phpcs:ignore
				wp_send_json( $data );
				break;
			...
			case 're_settlement_paypal_cp':
				...
				if ( ! empty( $ba_id ) ) {
					...
					/* Create order */
					$response_order_data = $this->api_create_order( $access_token, $bn_code, $intent, $request_id, $entry, $cart );
					if ( isset( $response_order_data['id'] ) && isset( $response_order_data['status'] ) && 'CREATED' == $response_order_data['status'] ) {
						$resource_id             = $response_order_data['id'];
						$correlation_id          = '';
						$payment_source          = array();
						...
						if ( 'COMPLETED' == $status && isset( $response_data['purchase_units'] ) ) {
							$this->save_acting_log( $response_data, $acting, $intent, $status, $amount, $order_id, $tracking_id );
							$purchase_units = $response_data['purchase_units'][0];
							if ( isset( $purchase_units['payments']['captures'] ) ) {
								$payments = $purchase_units['payments']['captures'][0];
							} elseif ( isset( $purchase_units['payments']['authorizations'] ) ) {
				...
				if ( 'COMPLETED' == $status ) {
					...
					$acting_status = 'ERROR';
					$class         = ' paypal-' . strtolower( $acting_status );
					$result       .= '<div class="paypal-settlement-admin paypal-error">' . __( $acting_status, 'usces' ) . '</div>'; // phpcs:ignore
					$result       .= '<table class="paypal-settlement-admin-table">
						<tr><th>' . __( 'Transaction amount', 'usces' ) . '</th>
							<td><input type="tel" class="settlement-amount" value="' . usces_crform( $amount, false, false, 'return', true ) . '" readonly />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>
						</tr>
			...
			case 'continuation_update':
				...
				if ( 'continuation' == $continue_data['status'] && 'cancellation' == $status ) {
					...
					if ( $new_charged_date < $tomorrow ) {
						$data['status']  = 'NG';
						$data['message'] = sprintf( __( 'The next settlement date must be after %s.', 'dlseller' ), $tomorrow ); // phpcs:ignore
						wp_send_json( $data );
					}
	...
	public function orderlist_settlement_status( $detail, $value, $key, $order_id ) {
		...
		if ( in_array( $acting_flg, $this->pay_method ) ) {
			...
			if ( ! empty( $acting_status ) ) {
				$class  = ' paypal-' . strtolower( $acting_status );
				$detail = '<td>' . $value . '<span class="acting-status' . $class . '">' . __( $acting_status, 'usces' ) . '</span></td>'; // phpcs:ignore
			}
		}
		return $detail;
	}
...
<div id="settlement_dialog" title="">
	...
	public function admin_member_info( $member, $member_metas, $usces_member_history ) {
		...
		$acting_opts = $this->get_acting_settings();
		if ( $this->is_activate_paypal_card() ) :
			/* Get Access Token */
			$access_token = $this->get_access_token();
	...
	public function admin_update_memberdata( $member_id, $res ) {
		if ( ! $this->is_activate_paypal_card() || false === $res ) {
			return;
		}

		if ( isset( $_POST['paypal_card_delete'] ) && 'delete' == $_POST['paypal_card_delete'] && isset( $_POST['paypal_payment_token_id'] ) ) { // phpcs:ignore
			/* Get Access Token */
			$access_token  = $this->get_access_token();
			$delete_status = $this->api_delete_payment_tokens( $access_token, $_POST['paypal_payment_token_id'] ); // phpcs:ignore
		}
	}
	...
	public function continue_member_list_page( $continue_action ) {
		if ( 'settlement_paypal_cp' == $continue_action ) {
			$member_id = ( isset( $_GET['member_id'] ) ) ? wp_unslash( $_GET['member_id'] ) : ''; // phpcs:ignore
			$order_id  = ( isset( $_GET['order_id'] ) ) ? wp_unslash( $_GET['order_id'] ) : ''; // phpcs:ignore
	...
	public function continue_member_settlement_info_page( $member_id, $order_id ) {
		...
		$curent_url    = esc_url( $_SERVER['REQUEST_URI'] ); // phpcs:ignore
		$navibutton    = '<a href="' . esc_url( $_SERVER['HTTP_REFERER'] ) . '" class="back-list"><span class="dashicons dashicons-list-view"></span>' . __( 'Back to the continue members list', 'dlseller' ) . '</a>'; // phpcs:ignore
...
<div class="edit_pagenav"><?php echo $navibutton; // phpcs:ignore ?></div>
<div id="datatable">
		...
		<td>
			...
			<tr>
				...
				<td>
					...
					for ( $i = 0; $i <= 10; $i++ ) {
						...
						if ( $year == (int) $contracted_year ) {
							echo '<option value="' . esc_html( $year ) . '" selected>' . esc_html( $year ) . '</option>';
						} else {
					...
					for ( $i = 1; $i <= 12; $i++ ) {
						$month = sprintf( '%02d', $i );
						if ( $i == (int) $contracted_month ) {
							echo '<option value="' . esc_html( $month ) . '" selected>' . esc_html( $month ) . '</option>';
						} else {
					...
					for ( $i = 1; $i <= 31; $i++ ) {
						$day = sprintf( '%02d', $i );
						if ( $i == (int) $contracted_day ) {
							echo '<option value="' . esc_html( $day ) . '" selected>' . esc_html( $day ) . '</option>';
						} else {
							echo '<option value="' . esc_html( $day ) . '">' . esc_html( $day ) . '</option>';
						}
					}
					echo '</select>';
					?>
				...
				<td>
					...
					if ( $charged_year == $this_year ) {
						echo '<option value="' . esc_html( $this_year ) . '" selected>' . esc_html( $this_year ) . '</option>';
					} else {
						echo '<option value="' . esc_html( $this_year ) . '">' . esc_html( $this_year ) . '</option>';
					}
					$next_year = (int) $this_year + 1;
					if ( $charged_year == $next_year ) {
						echo '<option value="' . esc_html( $next_year ) . '" selected>' . esc_html( $next_year ) . '</option>';
					} else {
						echo '<option value="' . esc_html( $next_year ) . '">' . esc_html( $next_year ) . '</option>';
					}
					echo '</select>-<select id="charged-month">';
					if ( 0 === (int) $charged_month ) {
						echo '<option value="0" selected></option>';
					} else {
						echo '<option value="0"></option>';
					}
			...
			<tr>
				...
				<?php
					...
					echo $dlseller_status_options; // phpcs:ignore
				?>
				</select></td>
...
<table id="mainDataTable" class="new-table order-new-table">
	...
	<tr>
		...
		<td><span id="settlement-status-<?php echo esc_attr( $num ); ?>"><span class="acting-status<?php echo esc_attr( $class ); ?>"><?php echo esc_html__( $acting_status, 'usces' ); // phpcs:ignore ?></span></span></td>
		<td>
...
<input name="usces_referer" type="hidden" id="usces_referer" value="<?php echo urlencode( $curent_url ); ?>" />
	...
	public function autobilling_message_htmlbody( $member_id, $order_id, $order_data, $response_data, $continue_data ) {
		...
		$message .= '</td><td style="padding: 12px; width: 67%; border: 1px solid #ddd;">';
		$message .= sprintf( _x( '%s', 'honorific', 'usces' ), $name ); // phpcs:ignore
		$message .= '</td></tr>';
	...
	private function get_continuation_ba_id( $member_id, $order_id = 0 ) {
		...
		if ( empty( $order_id ) ) {
			...
			$query             = $wpdb->prepare( "SELECT COUNT(*) FROM {$member_meta_table} WHERE `member_id` = %d AND `meta_key` LIKE %s", $member_id, 'paypal_continuation_%' ); // phpcs:ignore
			$ba_id             = $wpdb->get_var( $query ); // phpcs:ignore
		} else {
	...
	public function settlement_error_mail_message_htmlbody( $hook_args ) {
		extract( $hook_args ); // phpcs:ignore
		...
		$mail_body .= '<td style="padding: 12px; width: 67%; border: 1px solid #ddd;">' . sprintf( _x( '%s', 'honorific', 'usces' ), $name ) . '</td>'; // phpcs:ignore
		$mail_body .= '</tr>';
		$mail_body .= '<tr>';
	...
	private function get_regular_id( $order_id ) {
		...
		$reg_id = $wpdb->get_var( $query ); // phpcs:ignore
		if ( empty( $reg_id ) ) {
	...
	private function get_regular_ba_id( $member_id, $reg_id = 0 ) {
		...
		if ( empty( $reg_id ) ) {
			$member_meta_table = usces_get_tablename( 'usces_member_meta' );
			$query             = $wpdb->prepare( "SELECT COUNT(*) FROM {$member_meta_table} WHERE `member_id` = %d AND `meta_key` LIKE %s", $member_id, 'paypal_regular_%' ); // phpcs:ignore
			$ba_id             = $wpdb->get_var( $query ); // phpcs:ignore
		} else {
	...
	private function get_member_id( $order_id ) {
		...
		$member_id = $wpdb->get_var( $query ); // phpcs:ignore
		return $member_id;
	}
	...
	public function get_entry_log( $tracking_id ) {
		...
		$log_data = $wpdb->get_var( $query ); // phpcs:ignore
	...
	private function get_order_acting_flg( $order_id ) {
		...
		$order_payment_name = $wpdb->get_var( $query ); // phpcs:ignore
