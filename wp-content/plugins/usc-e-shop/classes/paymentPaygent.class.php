...
jQuery( document ).ready( function( $ ) {
	...
	<form action="" method="post" name="paygent_form" id="paygent_form">
		...
		<table class="settle_table">
			...
			<tr id="ex_stock_card_mode_paygent" class="explanation paygent_card_form"><td colspan="2">会員は購入の際に登録済みのカードで支払うことができます。カード番号はペイジェントのシステムに保存されます。利用する場合は、基本設定の「会員システム」を「利用する」に設定してください。<?php echo $stock_card_mode_msg; ?></td></tr>
			<tr class="paygent_card_form paygent_card_module">
...
function paidyPay() {
	...
	var payload = {
		...
		"buyer_data": {
			<?php echo $buyer_data; ?>
			<?php echo $additional_shipping_addresses; ?>
			<?php echo $number_of_points; ?>
		},
		"order": {
			"items": [
				<?php echo $items; ?>
