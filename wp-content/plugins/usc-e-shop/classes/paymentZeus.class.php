...
jQuery(document).ready(function($) {
	...
	public function settlement_update() {
		...
		if ( 'on' === $options['acting_settings']['zeus']['card_activate'] ) {
			...
			// if ( isset( $post_data['3dsecur'] ) && 1 === (int) $post_data['3dsecur'] ) {
			// if ( WCUtils::is_blank( $post_data['3ds_pattern'] ) ) {
			// $this->error_mes .= '※3Dセキュアの認証パターンを選択してください<br />';
			// }
			// }
		}
	...
	<form action="" method="post" name="zeus_form" id="zeus_form">
		<table class="settle_table">
			...
			<!--<tr class="card_zeus 3dsecur_zeus">
				<th><a class="explanation-label" id="label_ex_3ds_pattern_zeus">3Dセキュア認証パターン</a></th>
				<td><label><input name="3ds_pattern" type="radio" id="3ds_pattern_zeus_all" value="all"<?php checked( $pattern, 'all' ); ?> /><span>完全認証　（Attempt不可）</span></label><br />
					<label><input name="3ds_pattern" type="radio" id="3ds_pattern_zeus_a" value="a"<?php checked( $pattern, 'a' ); ?> /><span>通常認証Ａ（Attempt可）</span></label><br />
					<label><input name="3ds_pattern" type="radio" id="3ds_pattern_zeus_b" value="b"<?php checked( $pattern, 'b' ); ?> /><span>通常認証Ｂ（Attempt可）</span></label>
				</td>
			</tr>
			<tr id="ex_3ds_pattern_zeus" class="explanation card_zeus 3dsecur_zeus"><td colspan="2">3Dセキュアを利用する場合は認証パターンの設定が必要です。契約に従って指定する必要があります。</td></tr>-->
			<tr class="card_zeus">
			...
			<tr class="card_zeus">
				...
				<td><label><input name="howpay" type="radio" id="howpay_zeus_1" value="on"<?php checked( $howpay, 'on' ); ?> /><span>分割払いに対応する</span></label><br />
					<label><input name="howpay" type="radio" id="howpay_zeus_2" value="off"<?php checked( $howpay, 'off' ); ?> /><span>一括払いのみ</span></label>
				</td>
			</tr>
			<tr id="ex_howpay_zeus" class="explanation card_zeus"><td colspan="2">お客様が利用するクレジットカードのカード会社により、選択できる分割回数が異なります。<?php echo esc_html( $wcex_name ); ?>商品は常に「一括払い」で決済されます。</td></tr>
		</table>
		<table class="settle_table">
			<tr>
	...
	public function revival_orderdata( $order_id, $log_key, $acting ) {
		...
		$data = array();
		switch ( $acting ) {
			case 'acting_zeus_card': // phpcs:ignore
		...
		switch ( $mode ) {
			...
			case 'get_zeus_card':
				...
				if ( ! empty( $latest_log['order_no'] ) ) {
					...
					if ( 'TEST' === $order_ref['status'] ) {
						...
						$res        .= '<td><input type="tel" class="settlement-amount" value="' . intval( $latest_log['amount'] ) . '" disabled="disabled" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>'; // phpcs:ignore
						$res        .= '</tr>';
						$res        .= '</table>';
						...
						$res        .= '<table class="settlement-admin-table">';
						$res        .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>';
						$res        .= '<td><input type="tel" class="settlement-amount" id="amount_change" value="' . intval( $latest_log['amount'] ) . '" />' . __( usces_crcode( 'return' ), 'usces' ) . '<input type="hidden" id="amount_original" value="' . intval( $latest_log['amount'] ) . '" /></td>'; // phpcs:ignore
						$res        .= '</tr>';
						$res        .= '</table>';
						...
						if ( ! empty( $member_id ) ) {
							$res .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>';
							$res .= '<td><input type="tel" class="settlement-amount" id="amount_change" value="' . intval( $latest_log['amount'] ) . '" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>'; // phpcs:ignore
							$res .= '</tr>';
							$res .= '</table>';
							$res .= '<div class="settlement-admin-button">';
			...
			case 'sale_zeus_card':
				...
				if ( ! empty( $latest_log['order_no'] ) ) {
					...
					if ( false !== strpos( $page_sale, 'Success_order' ) ) {
						...
						$res          .= '<table class="settlement-admin-table">';
						$res          .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>';
						$res          .= '<td><input type="tel" class="settlement-amount" id="amount_change" value="' . intval( $amount ) . '" />' . __( usces_crcode( 'return' ), 'usces' ) . '<input type="hidden" id="amount_original" value="' . intval( $amount ) . '" /></td>'; // phpcs:ignore
						$res          .= '</tr>';
						$res          .= '</table>';
						$res          .= '<div class="settlement-admin-button">';
						$res          .= '<input id="change_settlement" type="button" class="button" value="金額変更" />';
			...
			case 'cancel_zeus_card':
				...
				if ( ! empty( $latest_log['order_no'] ) ) {
					...
					if ( false !== strpos( $page_cancel, 'SuccessOK' ) ) {
						...
						$res          .= '<table class="settlement-admin-table">';
						$res          .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>';
						$res          .= '<td><input type="tel" class="settlement-amount" id="amount_change" value="0" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>'; // phpcs:ignore
						$res          .= '</tr>';
						$res          .= '</table>';
						$res          .= '<div class="settlement-admin-button">';
						$res          .= '<input id="re_settlement" type="button" class="button" value="新規決済" />';
			...
			case 're_settlement_zeus_card':
				...
				if ( false !== strpos( $page, 'Success_order' ) ) {
					...
					$res .= '<table class="settlement-admin-table">';
					$res .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>';
					$res .= '<td><input type="tel" class="settlement-amount" id="amount_change" value="' . intval( $amount ) . '" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>'; // phpcs:ignore
					$res .= '</tr>';
					$res .= '</table>';
					$res .= '<div class="settlement-admin-button">';
					$res .= '<input id="re_settlement" type="button" class="button" value="新規決済" />';
				...
				if ( 'on' === $acting_opts['howpay'] && ! $member_page ) {
					...
					<tr id="cbrand_zeus">
						...
						<select name="offer[cbrand]" id="cbrand">
							<option value=""' . ( ( WCUtils::is_blank( $cbrand ) ) ? ' selected' : '' ) . '>--------</option>
							<option value="1"' . ( ( '1' === $cbrand ) ? ' selected' : '' ) . '>JCB</option>
							<option value="1"' . ( ( '1' === $cbrand ) ? ' selected' : '' ) . '>VISA</option>
							<option value="1"' . ( ( '1' === $cbrand ) ? ' selected' : '' ) . '>MASTER</option>
							<option value="2"' . ( ( '2' === $cbrand ) ? ' selected' : '' ) . '>DINERS</option>
							<option value="1"' . ( ( '1' === $cbrand ) ? ' selected' : '' ) . '>AMEX</option>
						</select>
						</td>
					</tr>
					<tr id="div_zeus">
						...
						<select name="offer[div_1]" id="brand1">
							<option value="01"' . ( ( '01' === $div ) ? ' selected' : '' ) . '>' . __( 'Single payment', 'usces' ) . '</option>
							<option value="99"' . ( ( '99' === $div ) ? ' selected' : '' ) . '>' . __( 'Libor Funding pay', 'usces' ) . '</option>
							<option value="03"' . ( ( '03' === $div ) ? ' selected' : '' ) . '>3' . __( '-time payment', 'usces' ) . '</option>
							<option value="05"' . ( ( '05' === $div ) ? ' selected' : '' ) . '>5' . __( '-time payment', 'usces' ) . '</option>
							<option value="06"' . ( ( '06' === $div ) ? ' selected' : '' ) . '>6' . __( '-time payment', 'usces' ) . '</option>
							<option value="10"' . ( ( '10' === $div ) ? ' selected' : '' ) . '>10' . __( '-time payment', 'usces' ) . '</option>
							<option value="12"' . ( ( '12' === $div ) ? ' selected' : '' ) . '>12' . __( '-time payment', 'usces' ) . '</option>
							<option value="15"' . ( ( '15' === $div ) ? ' selected' : '' ) . '>15' . __( '-time payment', 'usces' ) . '</option>
							<option value="18"' . ( ( '18' === $div ) ? ' selected' : '' ) . '>18' . __( '-time payment', 'usces' ) . '</option>
							<option value="20"' . ( ( '20' === $div ) ? ' selected' : '' ) . '>20' . __( '-time payment', 'usces' ) . '</option>
							<option value="24"' . ( ( '24' === $div ) ? ' selected' : '' ) . '>24' . __( '-time payment', 'usces' ) . '</option>
						</select>
						...
						<select name="offer[div_3]" id="brand3">
							<option value="01"' . ( ( '01' === $div ) ? ' selected' : '' ) . '>' . __( 'Single payment only', 'usces' ) . '</option>
						</select>
						</td>
					</tr>
				</table>';
	...
	public function member_update_settlement_form() {
		...
		if ( isset( $_POST['zeus_card_update'] ) ) {
			...
			if ( ! $check_verify_recaptcha ) {
				$usces->error_message .= __( 'Could not send successfully. Please try again.', 'usces' );
			} else {
...
<p><?php esc_html_e( 'Sorry, no posts matched your criteria.', 'usces' ); ?></p>
	...
	public function message_regular_order( $member_id ) {
		...
		$regular_order = $wpdb->get_results( $query_order, ARRAY_A ); // phpcs:ignore
		if ( 0 < count( $regular_order ) ) {
			foreach ( $regular_order as $regular_data ) {
				...
				$regular_order_data = $wpdb->get_results( $query, ARRAY_A ); // phpcs:ignore
...
<input name="usces_referer" type="hidden" id="usces_referer" value="<?php echo esc_url( $curent_url ); ?>" />
	...
	protected function get_order_meta_acting( $order_id ) {
		...
		$meta_value  = $wpdb->get_var( $query ); // phpcs:ignore
	...
	protected function get_order_status( $order_id ) {
		...
		$order_status = $wpdb->get_var( $query ); // phpcs:ignore
		return $order_status;
	}
